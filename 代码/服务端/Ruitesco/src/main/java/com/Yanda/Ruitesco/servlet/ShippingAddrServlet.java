...
//			String receiverPhone = request.getParameter("receiverPhone");
			...
			messageType = ship_service.addAddress(user_id, receiverName, receiverMobile, receiverProvince, receiverDistrict, receiverCity, receiverAddress, receiverZip);
		}
...
//			String phone = request.getParameter("receiverPhone");
			...
			messageType = ship_service.updateAddress(id, name, mobile, province, district, city, address, zip);
		}
		String json = gson.toJson(messageType);
		System.out.println(json);
		PrintWriter pw = response.getWriter();
	...
	public void SelectAllAddress(HttpServletRequest request, HttpServletResponse response) throws IOException {
		/**
		 * 功能：收货地址列表
		 * request: 无 session: username, userid
		 * 地址栏传参：mode="select"
		 * response: status 状态码(0成功, 1失败, 10管理员未登录), msg 反馈信息, data 获取的子类别信息集合与页码相关参数
		 * */
		HttpSession session = request.getSession();
