...
//		int category_id = 999999;
		if(username.equals("")) {
			...
			String host = "localhost:8080/image";
			String relative_address = "temp";
			String local_address = "D:/软件工程/课程设计/git/Rising_Shopping/image/";
			String path = host + relative_address;
			String fn = null;
			//根据类别生成存储路径
...
//						}
//						else {
							String localpath = local_address + relative_address;
							...
							if(!file.exists())
								file.mkdir();
							fi.write(new File(localpath, fn));
							messageResponse.setStatus(0);
							messageResponse.setMsg(relative_address + '/' + fn);
//						}
					}
				}
			}catch (Exception e) {
...
//		int id = Integer.parseInt(request.getParameter("id"));
		...
		try {
			...
			for(FileItem fi:fileItem) {
				if(fi.isFormField()) {
					...
					else if(fi.getFieldName().equals("productId"))
	...
	public MessageType<String> updateFileRoute(int categoryId, String image) {
		...
		String local_address = "D:/软件工程/课程设计/git/Rising_Shopping/image/";
		System.out.println("当前图片路径：" + local_address + image);
		MessageType<String> messageType = new MessageType<String>();
		String path = host + "image/";
		String localpath = local_address;
		...
		try {
			File file =new File(local_address + image);
			File fileFolder = new File(localpath + relative_path);
			if(!fileFolder.exists())
				fileFolder.mkdir();
			System.out.println("存储路径：" + localpath + relative_path + file.getName());
			if(!(local_address + image).equals(localpath + relative_path + file.getName())){
				if(file.renameTo(new File(localpath + relative_path + file.getName()))) {
					messageType = new MessageType<String>(0, "移动文件成功", relative_path + file.getName());
				}
				else
					messageType = new MessageType<String>(1, "移动文件失败", null);
			}
			else
				messageType = new MessageType<String>(0, "文件路径不变", relative_path + file.getName());
			}catch(Exception e) {
				e.printStackTrace();
		}
	...
	public void GetProductById(HttpServletRequest request, HttpServletResponse response) throws IOException {
		 ...
		 * response: status 状态码(0成功, 1失败, 10管理员未登录), msg 反馈信息, data 获取的子类别信息集合与页码相关参数
		 * */
		System.out.println("正在执行：GetProductById");
		int category_id = 0;
		MessageType<Object> messageType;
		if(request.getParameter("category_id") != null)
			category_id = Integer.parseInt(request.getParameter("category_id"));
		if(category_id == 0)
			messageType = new MessageType<Object>(2, "未获取到category_id", null);
		else
			messageType = product_service.GetProductByCategory(category_id);
		String json = gson.toJson(messageType);
		System.out.println(json);
		PrintWriter pw = response.getWriter();
		pw.write(json);
