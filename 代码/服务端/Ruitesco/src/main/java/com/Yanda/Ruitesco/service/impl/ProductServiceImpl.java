...
//			}
	...
	@Override
	public MessageType<Object> GetProductByCategory(int category_id) {
		// TODO Auto-generated method stub
		...
		while(!id.empty()) {
			List<Product> temp = product_dao.GetProductByCategory(id.peek());
			if(temp != null) {
				for(Product tp:temp)
					products.add(tp);
			}
			List<Category> temp2 = category_dao.GetCategory(id.peek());
			id.pop();
			if(temp2 != null) {
				for(Category cg:temp2)
					id.push(cg.getId());
			}
		}
		if(products.size() <= 0)
			messageType = new MessageType<Object>(1, "该类别不存在商品", null);
		else {
			Page page = new Page();
			page.CalculatePage(products.size());
			ProductData data = new ProductData(page, products);
			System.out.println("按类别获取商品成功！");
			messageType = new MessageType<Object>(0, "获取成功", data);
		}
		return messageType;
	}
}
