...
public class CategoryServiceImpl implements ICategoryService {
	...
	@Override
	public MessageResponse<Object> QueryAllLeaf() {
		// TODO Auto-generated method stub
		MessageResponse<Object> messageResponse = new MessageResponse<Object>();
		Stack<Integer> category_id = new Stack<Integer>();
		List<Category> result = new ArrayList<Category>();
		category_id.push(0);
		while(!category_id.empty()) {
			int temp_id = category_id.peek();
			category_id.pop();
			List<Category> temp = category_dao.GetCategory(temp_id);
			if(temp == null)
				result.add(category_dao.QueryCategory(temp_id));
			else {
				for(Category ca:temp) {
					category_id.add(ca.getId());
				}
			}
		}
		if(result.size() <= 0) {
			messageResponse.setStatus(1);
			messageResponse.setMsg("没有类别");
		}
		else {
			messageResponse.setStatus(0);
			messageResponse.setMsg(result);
